...
(function () {
  ...
  window.VQAuth = {
    ...
    // Merges the given keys into auth user_metadata (name, phone, branding, rates…).
    // Fires USER_UPDATED so the app-level user state refreshes automatically.
    updateUserMeta: async function (data) {
    ...
    // ── Branding ──────────────────────────────────────────────────────────────
    // One branding row per user (Settings → Branding tab). Both calls run on the
    // signed-in session, so the table's RLS policy scopes them to auth.uid().

    // Returns { data: row | null, error } — maybeSingle so a missing row is not an error.
    getBranding: async function (userId) {
      return sb
        .from('branding')
        .select('*')
        .eq('user_id', userId)
    ...
    // Inserts or updates the user's branding row in one call.
    saveBranding: async function (userId, fields) {
      var row = Object.assign({ user_id: userId, updated_at: new Date().toISOString() }, fields);
      return sb
        .from('branding')
